...
def axiom_generator_percept_sentence(t, tvec):
    ...
    for i in range(len(tvec)):
        if tvec[i]:
            perceptText = "{perception}{time}".format(perception=perception[i],time=t)
        else:
            perceptText = "~{perception}{time}".format(perception=perception[i],time=t)
        output.append(perceptText)
    axiom_str = "&".join(output)
    return axiom_str
...
def axiom_generator_initial_location_assertions(x, y):
    ...
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    axiom_str = "~{pit}&~{wumpus}".format(pit=pit_str(x,y),wumpus=wumpus_str(x,y))
    return axiom_str
...
def axiom_generator_pits_and_breezes(x, y, xmin, xmax, ymin, ymax):
    ...
    pitLocations = []
    "*** YOUR CODE HERE ***"
    cordinations = [(-1,0) ,(0,-1),(1,0),(0,1),(0,0)]
    for cord in cordinations:
        tempX,tempY = x + cord[0],y+cord[1]
        if (tempX >= xmin and tempX <= xmax) and (tempY >= ymin and tempY <= ymax):
            pitLocations.append(pit_str(tempX,tempY))
    axiom_str += "({Breeze}) <=> ({Pits})".format(Breeze=breeze_str(x,y),Pits=" | ".join(pitLocations)) 
    return axiom_str
...
def axiom_generator_wumpus_and_stench(x, y, xmin, xmax, ymin, ymax):
    ...
    for cord in cordinations:
        tempX,tempY = x + cord[0],y+cord[1]
        if (tempX >= xmin and tempX <= xmax) and (tempY >= ymin and tempY <= ymax):
            wumpusLocations.append(wumpus_str(tempX,tempY))
    axiom_str += "({Stench}) <=> ({Wumpus})".format(Stench=stench_str(x,y),Wumpus=" | ".join(wumpusLocations)) 
    return axiom_str
...
def axiom_generator_at_least_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    for x in range(xmin,xmax+1):
        for y in range(ymin,ymax+1):
            posibleLocation.append(wumpus_str(x,y))
    "*** YOUR CODE HERE ***"
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    axiom_str = " | ".join(posibleLocation)
    return axiom_str
...
def axiom_generator_at_most_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    for x in range(xmin,xmax+1):
        for y in range(ymin,ymax+1):
            posibleLocation.append(wumpus_str(x,y))
    for i in range(len(posibleLocation)):
        current = posibleLocation[i]
        temp = []
        for j in range(len(posibleLocation)):
            if posibleLocation[j] != current:
                temp.append("~{0}".format(posibleLocation[j]))
        output.append("({0} >> ({1}))".format(current,"&".join(temp)))
    axiom_str =  " & ".join(output)
    return axiom_str
...
def axiom_generator_only_in_one_location(xi, yi, xmin, xmax, ymin, ymax, t = 0):
    ...
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    current = state_loc_str(xi,yi,t)
    output = [current]
    for x in range(xmin,xmax+1):
        for y in range(ymin,ymax+1):
            if (x,y) != (xi,yi):
                output.append("~{0}".format(state_loc_str(x,y,t)))
    axiom_str =  " & ".join(output)
    return axiom_str
...
def axiom_generator_only_one_heading(heading = 'north', t = 0):
    ...
    for direction in directions:
        if direction.lower() == heading.lower():
            tempString = "Heading{0}{1}".format(direction,t)
        else:
            tempString =  "~Heading{0}{1}".format(direction,t)
...
def axiom_generator_have_arrow_and_wumpus_alive(t = 0):
    ...
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    axiom_str = "({arrow}&{wumpus})".format(arrow=state_have_arrow_str(t),wumpus=state_wumpus_alive_str(t))
    return axiom_str
...
def axiom_generator_location_OK(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = "(~{Pit} & ~({Wumpus} & {WumpusAlive})) <=> ({locationOk})".format(Pit=pit_str(x,y),Wumpus=wumpus_str(x,y),WumpusAlive=state_wumpus_alive_str(t),locationOk=state_OK_str(x,y,t))
    return axiom_str
...
def axiom_generator_breeze_percept_and_location_property(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = "({Location}) >> (({Breeze}) <=> ({BreezeLocation}))".format(Location=state_loc_str(x,y,t),Breeze=percept_breeze_str(t),BreezeLocation=breeze_str(x,y))
    return axiom_str
...
def axiom_generator_stench_percept_and_location_property(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = "({Location}) >> (({Stench}) <=> ({stenchLocation}))".format(Location=state_loc_str(x,y,t),Stench=percept_stench_str(t),stenchLocation=stench_str(x,y))
    return axiom_str
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    actions = ['({0} & (~{1} | {2} | {3} | {4} | {5} | {6}))'.format(state_loc_str(x,y,t),action_forward_str(t),percept_bump_str(t+1),
                                                     action_grab_str(t),action_shoot_str(t),action_turn_left_str(t),action_turn_right_str(t))]
    actionAndDirection = {
        "North":"({0} & ({1} & {2}))".format(state_loc_str(x,y-1,t),state_heading_north_str(t),action_forward_str(t)),
        "South": "({0} & ({1} & {2}))".format(state_loc_str(x,y+1,t),state_heading_south_str(t),action_forward_str(t)),
        "West": "({0} & ({1} & {2}))".format(state_loc_str(x + 1, y, t), state_heading_west_str(t), action_forward_str(t)),
        "East": "({0} & ({1} & {2}))".format(state_loc_str(x - 1, y, t), state_heading_east_str(t), action_forward_str(t))
    }
    
    coordAndDirection = [(-1,0,"East"),(0,-1,"North"),(1,0,"West"),(0,1,"South")]
    for item in coordAndDirection:
        if x + item[0] >= xmin and x + item[0] <= xmax and y + item[1] >= ymin and y + item[1] <= ymax:
            actions.append(actionAndDirection[item[2]])
    successorLocation = state_loc_str(x,y,t+1)
    axiom_str = '{0} <=> ({1})'.format(successorLocation,' | '.join(actions))
    return axiom_str
...
def axiom_generator_have_arrow_ssa(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    axiom_str = "({havingArrow} & ~{Shoot}) <=> ({havingArrowNextTime})".format(havingArrow= state_have_arrow_str(t),Shoot=action_shoot_str(t),havingArrowNextTime=state_have_arrow_str(t+1))
    return axiom_str
...
def axiom_generator_wumpus_alive_ssa(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    axiom_str = "({Wumpus} & ~{Scream}) <=> ({stillAlive})".format(
        Wumpus=state_wumpus_alive_str(t),
        Scream=percept_scream_str(t+1),
        stillAlive=state_wumpus_alive_str(t+1))
    return axiom_str
...
def axiom_generator_heading_north_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    NorthStraight = '({0} & ~({1} | {2}))'.format(state_heading_north_str(t),action_turn_left_str(t),action_turn_right_str(t))
    EastLeft = '({0} & {1})'.format(state_heading_east_str(t),action_turn_left_str(t))
    WestRight = '({0} & {1})'.format(state_heading_west_str(t),action_turn_right_str(t))
    axiom_str = '({0}) <=> ({1} | {2} | {3})'.format(state_heading_north_str(t+1),NorthStraight,EastLeft,WestRight)
...
def axiom_generator_heading_east_ssa(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    EastStraight = '({0} & ~({1} | {2}))'.format(state_heading_east_str(t),action_turn_left_str(t),action_turn_right_str(t))
    SouthLeft = '({0} & {1})'.format(state_heading_south_str(t),action_turn_left_str(t))
    NorthRight = '({0} & {1})'.format(state_heading_north_str(t),action_turn_right_str(t))
    axiom_str = '({0}) <=> ({1} | {2} | {3})'.format(state_heading_east_str(t+1),EastStraight,SouthLeft,NorthRight)
    return axiom_str
...
def axiom_generator_heading_south_ssa(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    SouthStraight = '({0} & ~({1} | {2}))'.format(state_heading_south_str(t),action_turn_left_str(t),action_turn_right_str(t))
    WestLeft = '({0} & {1})'.format(state_heading_west_str(t),action_turn_left_str(t))
    EastRight = '({0} & {1})'.format(state_heading_east_str(t),action_turn_right_str(t))
    axiom_str = '({0}) <=> ({1} | {2} | {3})'.format(state_heading_south_str(t+1),SouthStraight,WestLeft,EastRight)
    return axiom_str
...
def axiom_generator_heading_west_ssa(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    # axiom_str = "{currentWest} & ~{turnLeft} & ~{turnRight} <=> {nextWest}".format(
    #     currentWest=state_heading_west_str(t),
    #     turnLeft=action_turn_left_str(t),
    #     turnRight=action_turn_right_str(t),
    #     nextWest=state_heading_west_str(t+1)
    # )
    #axiom_str = wumpus_str(0,0)
    WestStraight = '({0} & ~({1} | {2}))'.format(state_heading_west_str(t),action_turn_left_str(t),action_turn_right_str(t))
    NorthLeft = '({0} & {1})'.format(state_heading_north_str(t),action_turn_left_str(t))
    SouthRight = '({0} & {1})'.format(state_heading_south_str(t),action_turn_right_str(t))
    axiom_str = '({0}) <=> ({1} | {2} | {3})'.format(state_heading_west_str(t+1),WestStraight,NorthLeft,SouthRight)
    return axiom_str
...
def axiom_generator_heading_only_north(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    axiom_str = "(~{West} & ~{East} & ~{South}) <=> ({North})".format(
...
def axiom_generator_heading_only_east(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    axiom_str = "({East}) <=> (~{West} & ~{South} & ~{North})".format(
        West=state_heading_west_str(t),
        East=state_heading_east_str(t),
        South=state_heading_south_str(t),
        North=state_heading_north_str(t)
...
def axiom_generator_heading_only_south(t):
    ...
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
    axiom_str = "({South}) <=> (~{West} & ~{East} & ~{North})".format(
        West=state_heading_west_str(t),
        East=state_heading_east_str(t),
        South=state_heading_south_str(t),
        North=state_heading_north_str(t)
...
def axiom_generator_only_one_action_axioms(t):
    ...
    for action in proposition_bases_actions:
        current = action
        nextActions = []
        for nextAction in proposition_bases_actions:
            if nextAction != current:
                nextActions.append("~{0}".format(actions[nextAction]))
        output.append("({0}) <=> ({1})".format(actions[current],"&".join(nextActions)))
    axiom_str =  " & ".join(output)
    return axiom_str
