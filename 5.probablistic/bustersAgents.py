...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        #Find the closest living ghost based on the distribution 
        for dist in livingGhostPositionDistributions:
            temp_pos = dist.argMax() #position with highest belief
            temp_distance = self.distancer.getDistance(pacmanPosition,temp_pos)
            
            if temp_distance < closestGhostDistance:
                closestGhostDistance = temp_distance
                closestGhostPos = temp_pos
        
        
        
        #Search for the action that makes Pacman go to the closest ghost
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            temp_distance = self.distancer.getDistance(successorPosition,closestGhostPos)
            
            if temp_distance < closestPacmanDistance:
                closestPacmanDistance = temp_distance
                bestAction = action
